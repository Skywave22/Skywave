...
class MainActivity : FlutterActivity() {
    ...
    override fun onCreate(savedInstanceState: android.os.Bundle?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                // Security fix: this receiver was previously registered with
                // RECEIVER_EXPORTED, meaning ANY other app on the device
                // could send a "media_control" broadcast and remote-control
                // playback (play/pause/seek) while SkyStream is in PiP.
                // RECEIVER_NOT_EXPORTED restricts delivery to broadcasts
                // sent from within this app's own process only — this
                // receiver only needs to hear from createPipActions()'s own
                // PendingIntents below, never from another app.
                registerReceiver(
                    receiver,
                    filter,
    ...
    override fun onPictureInPictureModeChanged(isInPictureInPictureMode: Boolean, newConfig: android.content.res.Configuration) {
        ...
        // Bug fix: flutterEngine can transiently be null during teardown
        // (e.g. activity destroyed while animating out of PiP). The old
        // flutterEngine!! force-unwrap crashed the app in that edge case;
        // a safe call + null check degrades gracefully instead.
        flutterEngine?.dartExecutor?.binaryMessenger?.let {
