...
class SettingsRepository {
  ...
  // Reuses the generic player-setting key/value store (same Hive box) so no
  // new storage plumbing is needed for this feature.

  static const String _kDownloadMaxConcurrent = 'download_max_concurrent';

  /// Max number of downloads allowed to run at the same time (1-5).
  /// Backed by background_downloader's native holding queue, so this limit
  /// is enforced by the OS-level downloader and persists across app restarts
  /// and backgrounding.
  Future<void> setDownloadMaxConcurrent(int value) async {
    await _storageService.setPlayerSetting(_kDownloadMaxConcurrent, value);
  }

  int getDownloadMaxConcurrent() {
    return _storageService.getPlayerSetting<int>(
          _kDownloadMaxConcurrent,
          defaultValue: 3,
        ) ??
        3;
  }
