...
class DownloadService {
  ...
  Future<void> init() async {
    ...
    // The holding queue caps how many downloads the native platform runs at
    // once (Android WorkManager / iOS URLSessions). It lives on the native
    // side, so throttling continues even if the app is backgrounded — unlike
    // a Dart-side TaskQueue, which would suspend with the app. The limit is
    // user-configurable via Settings > Downloads (default: 3).
    final maxConcurrent = _ref
        .read(settingsRepositoryProvider)
        .getDownloadMaxConcurrent();
  ...
  // --- Download Queue Manager (batch operations) --------------------------
  // These operate on every download currently tracked in the database that
  // is in a pausable/resumable/cancellable state. Used by the "Pause All /
  // Resume All / Cancel All" controls in the Downloads tab.

  Future<List<TaskRecord>> _activeRecords() async {
    final records = await FileDownloader().database.allRecords();
    return records
        .where(
          (r) =>
  ...
  /// Pauses every currently active/queued download. Tasks that don't support
  /// pausing (allowPause: false) are skipped silently — SkyStream always sets
  /// allowPause: true when enqueuing, so this is defensive only.
  Future<void> pauseAllDownloads() async {
    final records = await _activeRecords();
    for (final record in records) {
  ...
  /// Resumes every currently paused download.
  ...
  /// Cancels and removes every currently active/queued/paused download,
  /// mirroring the cleanup done by [cancelDownload] for a single task.
  ...
  /// Updates how many downloads may run at once (1-5), persists the choice,
  /// and re-applies it to the native holding queue immediately so it takes
  /// effect without an app restart.
  Future<void> setMaxConcurrentDownloads(int value) async {
    final clamped = value.clamp(1, 5);
    await _ref
        .read(settingsRepositoryProvider)
        .setDownloadMaxConcurrent(clamped);
    await FileDownloader().configure(
      globalConfig: [(Config.holdingQueue, (clamped, null, null))],
    );
  }

  int getMaxConcurrentDownloads() {
    return _ref.read(settingsRepositoryProvider).getDownloadMaxConcurrent();
  }
