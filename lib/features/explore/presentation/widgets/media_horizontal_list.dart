...
import '../../home/presentation/home_customization_provider.dart'
    show HomeCardDensity, HomeCardStyle;
...
class MediaHorizontalList extends StatefulWidget {
  ...
  /// Optional card-size multiplier (see [HomeCardDensity.scale]). Defaults to
  /// 1.0, i.e. exactly today's sizing — only the Home screen passes a
  /// user-configured value; Explore is unaffected.
  final double cardScale;

  /// Optional forced card shape. Defaults to [HomeCardStyle.asPublished],
  /// i.e. today's existing per-item portrait/landscape auto-detection.
...
class _MediaHorizontalListState extends State<MediaHorizontalList> {
  ...
  Widget build(BuildContext context) {
    ...
    // A forced card style (Settings > Customize Home) overrides the
    // per-item auto-detected aspect ratio; "asPublished" keeps today's
    // existing auto-detection behavior untouched.
    final bool effectiveIsPortrait = switch (widget.cardStyle) {
      HomeCardStyle.asPublished => _isPortrait,
      HomeCardStyle.portrait => true,
      HomeCardStyle.landscape => false,
    };

    final double baseCardWidth = isDesktop
        ? (effectiveIsPortrait ? 200.0 : 300.0)
        : (effectiveIsPortrait ? 130.0 : 200.0);
    final double cardWidth = baseCardWidth * widget.cardScale;

    final double imageHeight =
        cardWidth / (effectiveIsPortrait ? (2 / 3) : (16 / 9));
