...
import 'home_customization_provider.dart';
...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  /// Builds the hero-carousel sliver (or its top-padding fallback when there
  /// is no data to show one). Kept as its own method so the customizable
  /// section ordering below can slot it in wherever the user has placed it.
  Widget _buildHeroCarouselSliverEntry(
    ...
    return switch (state) {
      ...
      HomeSuccess(:final data) => RefreshIndicator(
        ...
        child: CustomScrollView(
          controller: _scrollController,
          slivers: [
            // Built-in sections render in the user's customized order
            // (Settings > Customize Home). Any section not present in
            // `sectionOrder` is hidden. Provider category rows (below) are
            // always appended after these and keep the provider's own order.
            for (final kind in ref.watch(
              homeCustomizationProvider.select((c) => c.sectionOrder),
            ))
              switch (kind) {
                HomeSectionKind.heroCarousel => SliverToBoxAdapter(
                  child: _buildHeroCarouselSliverEntry(
                    context,
                    data,
                    isWidescreen,
                  ),
                ...
                HomeSectionKind.continueWatching =>
                  (watchHistoryEnabled && history.isNotEmpty)
                      ? SliverToBoxAdapter(
                ...
                HomeSectionKind.syncedProgress =>
                  (syncedProgressAsync.asData?.value.isNotEmpty == true)
                      ? SliverToBoxAdapter(
              ...
              delegate: SliverChildBuilderDelegate(
                (context, index) {
                  ...
                  return MediaHorizontalList(
                    ...
                    heroTagPrefix: 'home',
                    cardScale: customization.cardDensity.scale,
