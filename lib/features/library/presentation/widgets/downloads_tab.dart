...
import 'download_queue_controls.dart';
  ...
  Widget build(BuildContext context) {
    ...
    return downloadsAsync.when(
      data: (downloads) {
        ...
        // Whether there's anything actionable for the queue-wide controls —
        // hides "Pause All" / "Resume All" when nothing is running/paused so
        // the bar doesn't show dead buttons for a list of finished downloads.
        final hasActive = downloads.any(
          (d) =>
              (activeProgress[d.task.metaData]?.status ?? d.status) ==
                  TaskStatus.running ||
              (activeProgress[d.task.metaData]?.status ?? d.status) ==
                  TaskStatus.enqueued,
        );
        final hasPaused = downloads.any(
          (d) =>
              (activeProgress[d.task.metaData]?.status ?? d.status) ==
              TaskStatus.paused,
        );
