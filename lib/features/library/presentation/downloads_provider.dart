...
class DownloadsNotifier extends _$DownloadsNotifier {
  ...
  // --- Queue-wide controls (Download Queue Manager) -----------------------

  /// Pauses every active/queued download. The individual TaskStatusUpdate
  /// events from FileDownloader drive the per-item state refresh, same as
  /// pausing a single item.
  ...
  /// Cancels every active/queued/paused download and refreshes the list
  /// immediately, since cancellation removes items rather than just
  /// changing their status.
  Future<void> cancelAll() async {
    await ref.read(downloadServiceProvider).cancelAllDownloads();
    state = AsyncData(await _refreshList());
  }
}


/// Persisted "max simultaneous downloads" setting (1-5), backed by the
/// native holding queue in [DownloadService]. Exposed as its own provider
/// (rather than folded into [GeneralSettings]) so the Downloads tab and
/// Settings screen can both watch it without pulling in unrelated settings.
@Riverpod(keepAlive: true)
class DownloadConcurrency extends _$DownloadConcurrency {
  @override
  int build() {
    return ref.read(downloadServiceProvider).getMaxConcurrentDownloads();
  }

  Future<void> setValue(int value) async {
    final clamped = value.clamp(1, 5);
    await ref.read(downloadServiceProvider).setMaxConcurrentDownloads(clamped);
    state = clamped;
  }
}
