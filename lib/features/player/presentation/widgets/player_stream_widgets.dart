  ...
  Widget _buildSlider({
    ...
    return SliderTheme(
      data: SliderThemeData(
        ...
        // activeTrackColor is intentionally left at its default: the
        // custom _GradientSliderTrackShape below paints the played portion
        // itself (accent -> accentAlt gradient) and ignores this color, but
        // some Slider internals (e.g. semantics) still read it, so keep a
        // sane fallback.
...
class PlayerPlayPauseButton extends StatelessWidget {
  ...
  /// When true, renders the "glassmorphic glow" skin instead of a flat
  /// [backgroundColor] fill: a diagonal accent -> accentAlt gradient circle
  /// with a soft outer glow. Used by the redesigned touch-center button.
  /// Ignored when [backgroundColor] is also provided.
  final bool glow;
  ...
  Widget _buildButton({required bool isPlaying, required bool isSpinning}) {
    ...
    if (backgroundColor != null) {
      ...
      decoration = BoxDecoration(
        shape: BoxShape.circle,
        gradient: HotstarPlayerStyle.glowButtonGradient,
        boxShadow: HotstarPlayerStyle.glowShadow(),
      );
    } else {
      decoration = null;
    }
...
/// Rounded slider track that paints the *played* (active) portion with the
/// [HotstarPlayerStyle.progressGradient] instead of a flat color, while
/// keeping the same geometry as [RoundedRectSliderTrackShape] so thumb /
/// overlay positioning is unaffected. The inactive portion still uses
/// [SliderThemeData.inactiveTrackColor] as normal.
class _GradientSliderTrackShape extends RoundedRectSliderTrackShape {
  ...
  void paint(
    ...
    // Inactive (unplayed) portion — flat color, same as stock behavior.
    ...
    // Active (played) portion — gradient shader instead of a flat color.
    ...
    if (!activeRect.isEmpty) {
      final Paint activePaint = Paint()
        ..shader = HotstarPlayerStyle.progressGradient.createShader(
          trackRect, // shader spans the *full* track so the gradient stays
          // visually consistent as the thumb moves, instead of
          // stretching/squashing to fit just the played segment.
        );
      context.canvas.drawRRect(
